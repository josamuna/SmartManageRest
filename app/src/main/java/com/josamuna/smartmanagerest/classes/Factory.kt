...
object Factory {
    ...
    fun executeConnection(connectionClass:ConnectionClass) : Connection?{
        ...
        return DriverManager.getConnection(String.format("jdbc:jtds:sqlserver://%s;databaseName=%s;user=%s;password=%s",
    ...
    fun foundEquipement(context: Context, stringKey: String): Equipment?{
        ...
        if (dataResult.next()){
            ...
            makeToastMessage(context, "No data corresponding with that search, please provide a correct key", ToastType.Long)
        }
